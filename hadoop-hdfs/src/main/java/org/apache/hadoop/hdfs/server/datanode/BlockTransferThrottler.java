...
class BlockTransferThrottler {
    ...
    private long bytesPerPeriod; // 每次周期能够发送的总byte数
    private long curPeriodStart; // 当前周期的开始时间
    private long curReserve;     // 保存了当前周期还能使用的剩余请求量
    ...
    synchronized void throttle(long numOfBytes) {
        ...
        /*如果本月剩余流量已用完，比如 5月份的流量为100MB，*/
        while (curReserve <= 0) {
            long now = System.currentTimeMillis();
            long curPeriodEnd = curPeriodStart + period;

            if (now < curPeriodEnd) {//你在20已经用完
                // Wait for next period so that curReserve can be increased.
                try {
                    wait(curPeriodEnd - now); // 需要等到下个月
                } catch (InterruptedException ignored) {
                }
            } else if (now < (curPeriodStart + periodExtension)) { // 买了本月的流量加油包
