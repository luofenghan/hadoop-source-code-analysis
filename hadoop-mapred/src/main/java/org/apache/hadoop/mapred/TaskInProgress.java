...
class TaskInProgress {
    ...
    private int numMaps; /*Map 任务数目，只对Reduce Task有效*/
    private int partition;/*该Task在task列表的索引*/
    private JobTracker jobtracker;/*JobTracker对象，用户获取全局时钟*/
    private TaskID id;/*task id，其后面加下标构成 TaskAttempt ID*/
    ...
    private long startTime = 0; /*TIP创建时间*/
    private long execStartTime = 0; /*第一个 Task Attempt开始运行时间*/
    private long execFinishTime = 0; /*最后一个运行成功的TaskAttempt完成时间*/
    private int completes = 0; /*Task Attempt 完成个数，实际只有两个值 0 和 1*/
    private boolean failed = false;/*该TIP是否运行失败*/
    private boolean killed = false;/*该TIP是否被杀死*/
    ...
    public boolean isCommitPending(TaskAttemptID taskId) {
        ...
        return t != null && t.getRunState() == TaskStatus.State.COMMIT_PENDING;
    }
    ...
    /**
     * 当前TaskTracker是否正在运行任务，如果活跃任务列表为空，则表示没有运行
     *
    ...
    public boolean shouldClose(TaskAttemptID taskid) {
         ...
         * 如果该任务所在的TaskInProgress已完成，那么则关闭它。
         */
        boolean close;
    ...
    /**
     * 判断一个TaskInProgress是否可运行：【没有失败】并且【TaskAttempt完成数为0】
     */
    boolean isRunnable() {
    ...
    /**
     * 记录了当前TaskInProgress 曾经在哪些TaskTracker运行失败过
     *
