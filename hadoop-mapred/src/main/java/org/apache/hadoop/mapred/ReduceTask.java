...
class ReduceTask extends Task {
    ...
    /*reduce阶段 Shuffle 字节数*/
    private Counters.Counter reduceShuffleBytes = getCounters().findCounter(Counter.REDUCE_SHUFFLE_BYTES);
    ...
    private final SortedSet<FileStatus> mapOutputFilesOnDisk = new TreeSet<>((a, b) -> {
    ...
    private <INKEY, INVALUE, OUTKEY, OUTVALUE> void runOldReducer(JobConf job,
        ...
        OutputCollector<OUTKEY, OUTVALUE> collector = (key, value) -> {
            out.write(key, value);
    ...
    class ReduceCopier<K, V> implements MRConstants {
         ...
         * 如果拷贝数据出错次数超过 maxFetchFailuresBeforeReporting ，
         * 可通过参数`mapreduce.reduce.shuffle.maxfetchfailures`设置，默认是10，
         * 则进行一些必要的检查，以决定是否杀死ReduceTask。
         */
        private int maxFetchFailuresBeforeReporting;

        /**
         * Maximum number of fetch failures before reducer aborts.
         * 如果拷贝数据出错次数超过 abortFailureLimit ，则杀死 ReduceTask，
         * 然后等待调度器重新执行
         */
         ...
         */
        private final List<MapOutput> mapOutputsFilesInMemory = Collections.synchronizedList(new LinkedList<MapOutput>());

        /**
         * The map for (Hosts, List of MapIds from this Host) maintaining
         * map output locations
         * 保存了TaskTracker Host与已完成任务列表的映射关系
         * <Hosts, List<MapOutputLocation>>
         */
        ...
        class ShuffleRamManager implements RamManager {
            ...
            /**
             * 用于检查内存能否 装下本次下载的数据，如果不能装下，则阻塞等待内存释放
             *
        ...
        private class MapOutputCopier extends Thread {
            ...
            /**
             * 通过HTTP从TaskTracker端下载 map output
             *
            ...
            private long copyOutput(MapOutputLocation loc) throws IOException, InterruptedException {
                ...
                Path filename = new Path(String.format(MapOutputFile.REDUCE_INPUT_FILE_FORMAT_STRING,
                        TaskTracker.OUTPUT, loc.getTaskId().getId()));

                // Copy the map output to a temp file whose name is unique to this attempt
                /*tmpMapOutput 文件名为：output/map_1.out-0*/
                Path tmpMapOutput = new Path(filename + "-" + id);

                // Copy the map output
                /*获得了一个MapOutput，下载的数据有可能在【内存】中或者在【本地文件】中 */
                ...
                synchronized (ReduceTask.this) {
                    ...
                    if (mapOutput.inMemory) {
                        ...
                        /*tmpMapOutput 文件名为：output/map_1.out-0 ==> output/map_1.out*/
            ...
            private MapOutput getMapOutput(MapOutputLocation mapOutputLoc, Path filename, int reduce)
                ...
                /*验证【期望】下载的MapTaskid是否与【实际】的MapTaskID一致*/
                ...
                if (shuffleInMemory) {
                    ...
                    mapOutput = shuffleInMemory(mapOutputLoc, connection, input, (int) decompressedLength, (int) compressedLength);
                } else {
                    if (LOG.isDebugEnabled()) {
                        LOG.debug("Shuffling " + decompressedLength + " bytes (" +
                                compressedLength + " raw bytes) " +
            ...
            private MapOutput shuffleInMemory(MapOutputLocation mapOutputLoc,
                                              ...
                                              InputStream input,
                                              int mapOutputLength, /*解压缩后的长度*/
                                              int compressedLength /*压缩后的长度*/)
                    throws IOException, InterruptedException {
        ...
        public ReduceCopier(TaskUmbilicalProtocol umbilical, JobConf conf,
                            TaskReporter reporter) throws ClassNotFoundException, IOException {
            ...
            Counters.Counter combineInputCounter = reporter.getCounter(Counter.COMBINE_INPUT_RECORDS);
            this.combinerRunner = CombinerRunner.create(conf, getTaskID(), combineInputCounter, reporter, null);
            if (combinerRunner != null) {
                combineCollector = new CombineOutputCollector(reduceCombineOutputCounter, reporter, conf);
            }
        ...
        public boolean fetchOutputs() throws IOException {
            ...
            LocalFSMerger localFSMergerThread;
            InMemFSMergeThread inMemFSMergeThread;

            /*该线程周期性地会通过RPC从TaskTracker上获取已完成的MapTask列表
            * 并将运行完成的MapTask放到列表mapLocations中
            * */
            GetMapEventsThread getMapEventsThread;

            for (int i = 0; i < numMaps; i++) {
                copyPhase.addPhase();       // add sub-phase per file
            }

            copiers = new ArrayList<>(numCopiers);

            // start all the copying threads
            for (int i = 0; i < numCopiers; i++) {
                MapOutputCopier copier = new MapOutputCopier(conf, reporter, reduceTask.getJobTokenSecret());
                copiers.add(copier);
                /*开始了下载，下载的数据会存到内存和本地文件中*/
                copier.start();
            }
            ...
            /*启动 拉取MapTask完成事件线程，从TaskTracker上获得已MapTask，然后唤醒MapOutputCopier进行map-out下载*/
            ...
            while (copiedMapOutputs.size() < numMaps && mergeThrowable == null) {
                ...
                synchronized (scheduledCopies) {
                    ...
                    while (hostsItr.hasNext()) {
                        ...
                        if (knownOutputsByLoc == null || knownOutputsByLoc.isEmpty()) {
                            continue;
                        }
                        ...
                        synchronized (knownOutputsByLoc) {
                            ...
                            while (locItr.hasNext()) {
                                ...
                                numInFlight++;
                                numScheduled++;
                                break; //we have a map from this host 已从该server上获得输出
                            }
                        }
                    }
                    ...
                    switch (event.getTaskStatus()) {
                        case SUCCEEDED: {
                            ...
                            URL mapOutputLocation = new URL(event.getTaskTrackerHttp() +
                                    ...
                                    "&map=" + taskId +
                                    "&reduce=" + getPartition());
                            List<MapOutputLocation> loc = mapLocations.computeIfAbsent(host, k -> Collections.synchronizedList(new LinkedList<>()));
                        ...
                        break;
                        case TIPFAILED: {/*TaskInProgress失败*/
