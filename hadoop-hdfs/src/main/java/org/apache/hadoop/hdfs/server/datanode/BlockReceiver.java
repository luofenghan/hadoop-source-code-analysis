...
class BlockReceiver implements java.io.Closeable, FSConstants {
    ...
    private int receivePacket() throws IOException {
        ...
        //read the header
        buf.getInt(); // 数据包的长度
        offsetInBlock = buf.getLong(); // 数据包在块中的偏移量
        long seqno = buf.getLong();    // 数据包的顺序号
        boolean lastPacketInBlock = (buf.get() != 0);  // 判断是否是最后一个数据包
        ...
        /*接受一个心跳包*/
        /*当客户端长时间没有传输数据时，就会发送这样的心跳包*/
        /*用于维护客户端与数据流管道上的各个数据节点的TCP连接*/
        if (len == 0) {
            ...
            while (running && datanode.shouldRun && !lastPacketInBlock) {
                ...
                try {
                    ...
                    if (lastPacketInBlock && !receiver.finalized) {
                        ...
                        datanode.data.finalizeBlock(block);// 提交数据块
                        datanode.myMetrics.incrBlocksWritten();
                        datanode.notifyNamenodeReceivedBlock(block, // 通知NameNode，完成了一个数据块接受
