...
public class Client {
    ...
    private class Connection extends Thread {
        /*TCP相关的成员变量*/
        private InetSocketAddress server;//IPC服务器的地址server（包含了建立TCP连接需要的IP地址/主机名和端口号信息）
        private Socket socket = null; //socket连接
        private DataInputStream in;
        private DataOutputStream out;

        /*IPC连接相关的成员变量*/
        private ConnectionHeader header;  //相对不变的连接消息头，是Socket连接建立后的第一条消息
        private final ConnectionId remoteId; //连接标识，用于IPC连接的复用
        private AtomicLong lastActivity = new AtomicLong();// 记录最后一次IO发生的时间
        private AtomicBoolean shouldCloseConnection = new AtomicBoolean();  // 当需要关闭连接时（停止Connection线程），置为false。
        private IOException closeException; // 记录异常关闭的原因

        /*远程调用相关的变量*/
        private Hashtable<Integer, Call> calls = new Hashtable<Integer, Call>();//保存了目前IPC连接上的所有远程调用对象
            ...
            try {
                ...
                while (true) {
                    ...
                    if (useSasl) {
                        ...
                        try {
                            continueSasl = ticket.doAs((PrivilegedExceptionAction<Boolean>) () -> setupSaslConnection(in2, out2));
                        } catch (Exception ex) {
                            if (rand == null) {
                                rand = new Random();
                            }
