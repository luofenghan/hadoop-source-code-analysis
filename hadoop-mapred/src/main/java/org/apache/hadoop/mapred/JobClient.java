...
public class JobClient extends Configured implements MRConstants, Tool {
     ...
     * <p>
     * 安装用户使用命令行选项 -libjars, -files, -archives 提交的 jobconf
     *
    ...
    public RunningJob submitJobInternal(final JobConf job) throws FileNotFoundException,
         ...
         */
        return ugi.doAs((PrivilegedExceptionAction<RunningJob>) () -> {
            JobConf jobCopy = job;
            /*新建一个本地暂存目录*/
            ...
            /*
            * 默认的提交目录 /tmp/hadoop/mapred/staging/.staging/
            *
            */
            ...
            try {
                ...
                try (FSDataOutputStream out = FileSystem.create(fs, submitJobFile, new FsPermission(JobSubmissionFiles.JOB_FILE_PERMISSION))) {
