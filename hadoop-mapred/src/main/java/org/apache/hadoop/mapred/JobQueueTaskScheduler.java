...
class JobQueueTaskScheduler extends TaskScheduler {
    ...
    public synchronized List<Task> assignTasks(TaskTracker taskTracker) throws IOException {
        ...
        final int clusterNumTaskTrackers = clusterStatus.getTaskTrackers();/*当前集群中TasKTracker的数量*/
        final int clusterMapCapacity = clusterStatus.getMaxMapTasks();/*集群中最大的map任务数*/
        final int clusterReduceCapacity = clusterStatus.getMaxReduceTasks();/*集群中最大的reduce任务数*/
        ...
        //
        /*获取当前TaskTracker下map 和reduce 任务的统计信息*/
        final int trackerMapCapacity = taskTrackerStatus.getMaxMapSlots();/*当前TaskTracker的map容量*/
        ...
        //
        int remainingReduceLoad = 0;  /*集群上剩余的reduce任务数*/
        int remainingMapLoad = 0;   /*集群上剩余的map任务数   */
        ...
        for (int i = 0; i < availableMapSlots; ++i) {
            synchronized (jobQueue) {
                for (JobInProgress job : jobQueue) {
                    ...
                    /*尝试调度一个 node-local 或者 rack-local map任务*/
                    t = job.obtainNewNodeOrRackLocalMapTask(taskTrackerStatus, clusterNumTaskTrackers, taskTrackerManager.getNumberOfUniqueHosts());
                    if (t != null) {
                        assignedTasks.add(t);
                        ++numLocalMaps;
                    ...
                    // Try to schedule a node-local or rack-local Map task
                    t = job.obtainNewNonLocalMapTask(taskTrackerStatus, clusterNumTaskTrackers, taskTrackerManager.getNumberOfUniqueHosts());
        ...
        if (availableReduceSlots > 0) {
            ...
            synchronized (jobQueue) {
                for (JobInProgress job : jobQueue) {
                    ...
                    Task t =job.obtainNewReduceTask(taskTrackerStatus, clusterNumTaskTrackers, taskTrackerManager.getNumberOfUniqueHosts());
                    if (t != null) {
                        assignedTasks.add(t);
                        break;
                    }
