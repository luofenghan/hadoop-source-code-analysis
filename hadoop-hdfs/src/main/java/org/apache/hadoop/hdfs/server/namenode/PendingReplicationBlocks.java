...
class PendingReplicationBlocks {
    /*
    * 从 【neededReplications】 移动过来 的待复制的复本
    * */
    private Map<Block, PendingBlockInfo> pendingReplications;

    /*当复制请求的产生时间和当前时间间隔大于timeout，就会放入【timedOutItems中】*/
    /*所以，默认情况下，如果复制操作没有成功，在5~10分钟后，这些操作会进入【timedOutItems】，
    * 最终又回到 【neededReplications】 中，重新产生复制请求。
    * 通过这种手段，NameNode可以防止数据块复制请求执行失败造成的复制丢失问题。
    * */
    ...
    static class PendingBlockInfo {
        private long timeStamp;/*复制请求的产生时间*/
        private int numReplicasInProgress;/*复制复本系数*/
    ...
    void metaSave(PrintWriter out) {
        synchronized (pendingReplications) {
            ...
            for (Map.Entry<Block, PendingBlockInfo> entry : pendingReplications.entrySet()) {
