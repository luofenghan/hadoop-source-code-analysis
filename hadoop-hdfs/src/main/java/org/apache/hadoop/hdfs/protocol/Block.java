...
public class Block implements Writable, Comparable<Block> {
    ...
    static {                                      // register a ctor
        WritableFactories.setFactory(Block.class, Block::new);
    }
    ...
    public static boolean isBlockFilename(File f) {
        ...
        return name.startsWith("blk_") && name.indexOf('.') < 0;
    }

    private static long filename2id(String name) {
        return Long.parseLong(name.substring("blk_".length()));
    }

    private long blockId; // 数据块的唯一标识，即数据块的ID号
    private long numBytes; // 数据块包含的文件数据大小
    // 数据块的版本号，或数据块时间戳。用于数据一致性检查，例如当成功在数据块后面添加数据之后，就会修改数据块的版本号
    // 具有相同数据块标识，但版本号不一致的数据块，最少有一个是无效的、需要被删除的数据块
    ...
    // Comparable
    /////////////////////////////////////
    private static void validateGenerationStamp(long generationstamp) {
