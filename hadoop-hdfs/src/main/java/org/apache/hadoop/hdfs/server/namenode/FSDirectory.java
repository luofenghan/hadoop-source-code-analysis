 ...
 * and logged to disk.
 * FSDirectory是门面模式的运用，为子系统提供的一组接口提供了一个一致的界面，使得使用这个子系统更加容易。
 * <p></p>
 * FSDirectory有60多个成员函数，基本上远程接口ClientProtocol的方法都可以在这里找到
 *
 *************************************************/
class FSDirectory implements FSConstants, Closeable {

    final FSNamesystem namesystem; /*是整个NameNode 的门面，用于访问数据块管理的相关功能*/
    final INodeDirectoryWithQuota rootDir;/*整个文件系统的根目录，*/
    FSImage fsImage;/*提供了命名空间镜像和编辑日志的相关功能*/
    private boolean ready = false;/*当FSDirectory.loadFSImage()完成命名空间镜像和编辑日志加载后，修改为true，这是可以对目录树进行操作*/
    ...
    /**
     * FSImage.loadFSImage()在读入命名空间镜像，处理INode信息时，每读取一条记录，
     * 都会调用该方法往目录树添加对应的INode。
     *
     * @param src
     * @param parentINode
     * @param permissions
    ...
    boolean exists(String src) {
        ...
        synchronized (rootDir) {
            ...
            return inode != null && (inode.isDirectory() || ((INodeFile) inode).getBlocks() != null);
        }
    }
    ...
    void replaceNode(String path, INodeFile oldnode, INodeFile newnode)
        ...
        synchronized (rootDir) {
             ...
             *  newnode [block1, block2, block3]
             *
             *  blocksMap<block1, newnode>,<block2, newnode>,<block3, newnode>
             */
