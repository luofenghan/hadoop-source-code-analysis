...
public abstract class FileInputFormat<K, V> implements InputFormat<K, V> {
    ...
    public enum Counter {BYTES_READ}
    ...
    public InputSplit[] getSplits(JobConf job, int numSplits) throws IOException {
        ...
        for (FileStatus file : files) {
            ...
            if ((length != 0) && isSplitable(fs, path)) {/*文件可切割*/
    ...
    protected String[] getSplitHosts(BlockLocation[] blkLocations, long offset, long splitSize, NetworkTopology clusterMap)
        ...
        BlockLocation currentBlock = blkLocations[startIndex];
        /*当前偏移量下在该块中能够读取的字节数*/
        long bytesInThisBlock = currentBlock.getOffset() + currentBlock.getLength() - offset;

        //If this is the only block, just return
        if (bytesInThisBlock >= splitSize) {
            return currentBlock.getHosts();
        }
        /*当前块剩余的字节数 小于 splitSize， 则需要增加新块*/
        ...
        for (index = startIndex; index <= endIndex; index++) {
            ...
            for (String topo : allTopos) {
                ...
                if (nodeInfo == null) {
                    ...
                    parentNodeInfo = racksMap.computeIfAbsent(parentNode, k -> new NodeInfo(parentNode));
