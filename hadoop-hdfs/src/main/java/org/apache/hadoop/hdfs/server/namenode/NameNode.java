...
public class NameNode implements ClientProtocol, DatanodeProtocol, NamenodeProtocol,
        FSConstants, RefreshAuthorizationPolicyProtocol, RefreshUserMappingsProtocol {
     ...
     * <p>
     * 如果serviceRpcServer存在，那么 DatanodeProtocol 和 NamenodeProtocol 上的
     * 远程调用都必须使用 serviceRpcServer，
     * 否则，所有的IPC请求都使用 server处理
     */
    ...
    public void create(String src,
        ...
        String clientMachine = getClientMachine();  /*获得发起请求的机器*/
        if (stateChangeLog.isDebugEnabled()) {
            stateChangeLog.debug("*DIR* NameNode.create: file "
                    + src + " for " + clientName + " at " + clientMachine);
        }
        if (!checkPathLength(src)) {/*检查文件路径长度和深度*/
    ...
    /**
     * 这个方法参数不多，业务逻辑需要的参数，如数据块大小、打开文件的客户端等
     * 都可以在 INodeFileUnderConstruction对象中找到
     *
     * @param src           包括申请分配新的数据块的文件
     ...
     * 当HDFS打开支持Append操作开关后，数据节点注册后，
     * 会报告节点上处于客户端【写状态的数据块信息】
     * 也就是上次数据节点退出时，写操作还没有结束，没提交的数据块。
     * 这些数据块对应的数据文件，保存在“${dfs.data.dir}/blocksBeingWritten”
     */
