...
public class DistributedCache {
     ...
     * @deprecated Please use {@link #addFileToClassPath(Path, Configuration, FileSystem)}
     ...
     * @param conf    Configuration that contains the classpath setting
     * @deprecated Please use {@link #addArchiveToClassPath(Path, Configuration, FileSystem)}
     * instead.  The {@code FileSystem} should be obtained within an
     * appropriate {@code doAs}.
     */
    @Deprecated
    ...
    public static boolean checkURIs(URI[] uriFiles, URI[] uriArchives) {
        ...
        if (uriFiles != null) {
            for (URI uriFile : uriFiles) {
                String fragment = uriFile.getFragment();
                if (fragment == null) {
                    return false;
                }
        ...
        if (uriArchives != null) {
            for (URI uriArchive : uriArchives) {
                String fragment = uriArchive.getFragment();
                if (fragment == null) {
                    return false;
                }
                String lowerCaseFragment = fragment.toLowerCase();
