 ...
 * JobInProgress维护了作业的运行时信息。
 * 它在作业运行过程中一直存在，主要用于跟踪正在运行作业的运行状态和进度。
 * ***********************************************************
 */
public class JobInProgress {
    ...
    /*允许每个TaskTracker上失败的Task个数，默认为4*/
    /*通过参数mapred.max.tracker.failures设置*/
    /*当该作业在某个TaskTracker上的失败个数超过该值时，会将该节点添加到该作业的黑名单中*/
    final int maxTaskFailuresPerTracker;

    private static float DEFAULT_COMPLETED_MAPS_PERCENT_FOR_REDUCE_SLOWSTART = 0.05f;
    /*当有5%的Map 任务完成后，才开始调度Reduce任务*/
    int completedMapsForReduceSlowstart = 0;
    /*允许Map 任务失败比例上限，通过参数mapred.max.map.failures.percent设置*/
    ...
    int speculativeMapTasks = 0;/*推测执行的map任务数*/

    int speculativeReduceTasks = 0;
    int failedMapTIPs = 0;/*失败的TaskInProgress数目，意味着对应的输入数据将被丢弃，不会产生最终结果*/
    int failedReduceTIPs = 0;
    private volatile boolean launchedCleanup = false;/*是否已经启动Cleanup task*/
    ...
    /*节点与上面【未运行TaskInProgress】的映射关系*/
    Map<Node, List<TaskInProgress>> nonRunningMapCache;

    /*节点与上面【正在运行的TaskInProgress】的映射关系，一个任务获得调度机会，其TIP会添加进来*/
    Map<Node, Set<TaskInProgress>> runningMapCache;

    /*指不需要考虑数据本地性的任务集合，即InputSplit Location为空，
    并且该集合中的任务也没有被运行*/
    final List<TaskInProgress> nonLocalMaps;

    /*不考虑数据本地性 并且正在运行任务的集合*/
    Set<TaskInProgress> nonLocalRunningMaps;

    /*按照TaskAttempt失败次数进行排序的TIP集合*/
    ...
    private static final Comparator<TaskInProgress> failComparator = (t1, t2) -> {
        if (t1 == null) return -1;
        if (t2 == null) return 1;
    ...
    private final int maxLevel; /*最大值为2，采用二层网络拓扑结构，数据中心一层不考虑*/
    ...
    JobInProgress(JobTracker jobtracker, final JobConf default_conf,
        ...
        try {
            ...
            this.status = new JobStatus(jobId, 0.0f, 0.0f, JobStatus.PREP);
            this.status.setUsername(jobInfo.getUser().toString());//cwc
            ...
            /*打开作业提交目录的HDFS文件系统*/
            fs = userUGI.doAs((PrivilegedExceptionAction<FileSystem>) () -> jobSubmitDir.getFileSystem(default_conf));
            ...
            /*从HDFS上下载 job.xml 到本地，重命名为 jobId.xml*/
            fs.copyToLocalFile(jobFilePath, localJobFile);
            /*读取作业的配置文件*/
            conf = new JobConf(localJobFile);
            if (conf.getUser() == null) {
                this.conf.setUser(user);/*cwc*/
            }
    ...
    private Map<Node, List<TaskInProgress>> createCache(TaskSplitMetaInfo[] splits, int maxLevel) throws UnknownHostException {
        Map<Node, List<TaskInProgress>> cache = new IdentityHashMap<>(maxLevel);

        Set<String> uniqueHosts = new TreeSet<>();
        for (int i = 0; i < splits.length; i++) {
            String[] splitLocations = splits[i].getLocations();
            if (splitLocations == null || splitLocations.length == 0) {
                /*如果没有splitLocation，说明是不需要考虑【数据本地性】的任务*/
    ...
    public synchronized void initTasks() throws IOException, KillInterruptedException, UnknownHostException {
        ...
        try {
            userUGI.doAs((PrivilegedExceptionAction<Object>) () -> {
        ...
        for (int i = 0; i < numMapTasks; ++i) {
            ...
            maps[i] = new TaskInProgress(jobId, jobFile,/*job.xml*/
        ...
        /*作业结束标志性任务，完成一些清理工作，一个用于map，另一个用于reduce*/
        ...
        /*创建两个setup TIP，一个是Map，另一个是Reduce*/
    ...
    TaskSplitMetaInfo[] createSplits(org.apache.hadoop.mapreduce.JobID jobId)
            throws IOException {
        return SplitMetaInfoReader.readSplitMetaInfo(jobId, fs, jobtracker.getConf(), jobSubmitDir);
    }
     ...
     */
    /**
     * 封装了所有调度器公用的任务选择策略实现，主要思想是：
     * 1. 优先选择【运行失败的任务】，以让其快速获取重新运行的机会；
     * 2. 其次实按照【数据本地行策略】选择尚未运行的任务；
     * 3. 最后查找正在运行的任务，尝试为【“拖后腿”的任务】启动备份任务。
     *
     * @param tts
     * @param clusterSize
    ...
    public synchronized Task obtainNewMapTask(TaskTrackerStatus tts,
        ...
        if (status.getRunState() != JobStatus.RUNNING) {
            ...
            try {
                throw new IOException("state = " + status.getRunState());
   ...
   */
    ...
    public synchronized Task obtainNewNodeOrRackLocalMapTask(TaskTrackerStatus tts,
                                                             int clusterSize, /*指TaskTracker的个数*/
                                                             int numUniqueHosts /*指机器节点的个数*/) throws IOException {
        if (!tasksInited) {
            LOG.info("Cannot create task split for " + profile.getJobID());
            try {
                throw new IOException("state = " + status.getRunState());
    ...
    public synchronized Task obtainNewReduceTask(TaskTrackerStatus tts,
        ...
        if (((estimatedReduceInputSize) > reduce_input_limit) && (reduce_input_limit > 0L)) {
     ...
     * to the blacklist if too many trackers in the cluster i.e.
    ...
    synchronized void addTrackerTaskFailure(String trackerName, TaskTracker taskTracker) {
        if (flakyTaskTrackers < (clusterSize * CLUSTER_BLACKLIST_PERCENT)) {
            ...
            if (trackerFailures == maxTaskFailuresPerTracker) {
                ++flakyTaskTrackers;
    ...
    private int getTrackerTaskFailures(String trackerName) {
        String trackerHostName = convertTrackerNameToHostName(trackerName);
        return trackerToFailuresMap.getOrDefault(trackerHostName, 0);
    }
     ...
     * 将一个map任务 TaskInProgress 添加到 正在运行map任务列表中
     *
     * @param tip the tip that needs to be scheduled as running 需要被调度运行的任务
     */
    protected synchronized void scheduleMap(TaskInProgress tip) {
        ...
        if (splitLocations == null || splitLocations.length == 0) {
            /*如何输入分片为空，则将TaskInProgress放入为运行任务列表中*/
        ...
        for (String host : splitLocations) {
            ...
            for (int j = 0; j < maxLevel; ++j) {
                ...
                Set<TaskInProgress> hostMaps = runningMapCache.computeIfAbsent(node, k -> new LinkedHashSet<>());
                hostMaps.add(tip);
                node = node.getParent();
            }
        }
     ...
     * @param ttStatus        请求运行任务的TaskTracker节点状态
     * @param numUniqueHosts  number of unique hosts that run task trackers 运行TaskTracker的主机数量
     * @param removeFailedTip 是否移除失败的TaskInProgress
     */
    private synchronized TaskInProgress findTaskFromList(Collection<TaskInProgress> tips,
        ...
        for (Iterator<TaskInProgress> iter = tips.iterator(); iter.hasNext(); ) {
            ...
            /*如果当前TaskInProgress【可运行】 并且 【TaskInProgress没有正在运行任务】*/
            if (tip.isRunnable() && !tip.isRunning()) {
                // check if the tip has failed on this host
                /*检查TIP曾经是否在此TaskTracker上失败过*/
                if (!tip.hasFailedOnMachine(ttStatus.getHost()) ||
                        /*如果失败过，那么判断是否在所有节点上都失败过*/
                        tip.getNumberOfFailedMachines() >= numUniqueHosts) {
                    /*如果没有在该TaskTracker上失败过，或者TIP在所有的节点上都失败过，则返回该TIP*/
                    iter.remove();
                    return tip;
     ...
     * Find new map task
     *
     * @param taskTrackerStatus The task tracker that is asking for a task
     * @param clusterSize       集群中TaskTracker的个数
     * @param numUniqueHosts    运行TaskTracker的机器节点数
     * @param avgProgress       在作业中该类型任务的平均进度
     ...
     */
    private synchronized int findNewMapTask(final TaskTrackerStatus taskTrackerStatus,
        ...
        String taskTracker = taskTrackerStatus.getTrackerName();
        TaskInProgress tip;
        ...
        this.clusterSize = clusterSize;
        /*：*/
        /*【第一步：合法性检查1】--如果一个作业在某个节点上失败任务数目超过一定阈值，则不会为其分配任务*/
        if (!shouldRunOnTaskTracker(taskTracker)) {
            return -1;
        }

        /*【第一步：合法性检查2】--检查确保该TaskTracker有足够的资源去运行该作业的任务，如果空间不足，则不会为其分配任务*/
        long outSize = resourceEstimator.getEstimatedMapOutputSize();
        long availSpace = taskTrackerStatus.getResourceStatus().getAvailableSpace();
        if (availSpace < outSize) {/*当前节点可用空间 小于 该节点map任务的输出*/
            LOG.warn("No room for map task. Node " + taskTrackerStatus.getHost() +
                    " has " + availSpace +
        ...
        //  0) Schedule a failed task without considering locality 调度失败的任务不会考虑本地性
        //  1) Schedule non-running tasks 调度没有运行的任务
        //  2) Schedule speculative tasks 调度推测任务
        //  3) Schedule tasks with no location information 调度没有本地信息的任务
        ...
        //    machine
        /*【第二步：从failedMaps中选择任务】--失败次数越多的任务，被调度的机会越大，为了让失败任务快速得到重新运行的机会，不需要考虑本地性*/
        tip = findTaskFromList(failedMaps, taskTrackerStatus, numUniqueHosts, false);
        if (tip != null) {
        ...
        /*获得当前JobTracker所在的节点映射*/

        /*【第三步：从nonRunningMapCache中选择任务】--采用的任务选择策略完全遵循数据本地行策略，
        *  即任务选择优先级从高到低依次为non-local , rack-local, off-switch*/
        Node node = jobtracker.getNode(taskTrackerStatus.getHost());
        ...
        if (node != null) {
            ...
            int maxLevelToSchedule = Math.min(maxCacheLevel, maxLevel);//2
            for (level = 0; level < maxLevelToSchedule; ++level) {
                /*选择该节点上未运行的TIP集合*/
                List<TaskInProgress> cacheForLevel = nonRunningMapCache.get(key);
                if (cacheForLevel != null) {
                    tip = findTaskFromList(cacheForLevel, taskTrackerStatus, numUniqueHosts, level == 0);
                    if (tip != null) {
                        // Add to running cache
                        scheduleMap(tip);

                        // remove the cache if its empty
                        if (cacheForLevel.isEmpty()) {
        ...
        /*【第三步：从nonRunningMapCache中选择任务2】--开始【跨机架】查找可运行的任务*/
        ...
        for (Node parent : nodesAtMaxLevel) {
            ...
            if (cache != null) {
                tip = findTaskFromList(cache, taskTrackerStatus, numUniqueHosts, false);
                if (tip != null) {
        ...
        /*【第四步：从nonLocalMaps中选择任务】--由于nonLocalMaps中的任务没有输入数据，因此无需考虑数据本地性*/
        tip = findTaskFromList(nonLocalMaps, taskTrackerStatus, numUniqueHosts, false);
        if (tip != null) {
            // Add to the running list
            scheduleMap(tip);
        ...
        // 2) Running TIP :
        //
        /*【第五步：从runningMapCache中选择任务】--遍历 runningMapCache 列表，查找是否存在正在运行且“拖后腿”的任务，如果有，则为其启动一个备份任务。*/
        if (hasSpeculativeMaps) {
            ...
            if (node != null) {
                ...
                for (int level = 0; level < maxLevel; ++level) {
                    ...
                    if (cacheForLevel != null) {
                        tip = findSpeculativeTask(cacheForLevel, taskTrackerStatus,
            ...
            for (Node parent : nodesAtMaxLevel) {
                ...
                if (cache != null) {
                    tip = findSpeculativeTask(cache, taskTrackerStatus, avgProgress,
                            currentTime, false);
                    if (tip != null) {
            ...
            /*【第五步：从nonLocalRunningMaps 中选择任务】--查找是否存在正在运行且“拖后腿”的任务，如果有，则为其启动一个备份任务。*/
            tip = findSpeculativeTask(nonLocalRunningMaps, taskTrackerStatus, avgProgress,
                    currentTime, false);
            if (tip != null) {
                LOG.info("Choosing a non-local task " + tip.getTIPId()
                        + " for speculation");
    ...
    private synchronized int findNewReduceTask(TaskTrackerStatus tts,
        ...
        /*从nonRunningReduces列表中选择任务，无需考虑数据本地行，只需要一次遍历该列表任务即可*/
    ...
    private synchronized void terminate(int jobTerminationState) {
        ...
        if ((status.getRunState() == JobStatus.RUNNING) ||
            ...
            // kill all TIPs.
            //
            for (TaskInProgress aSetup : setup) {
                aSetup.kill();
            }
     ...
     * 标记某个任务失败，但是没有状态对象
     *
     * @param tip         任务的TaskInProgress对象
     * @param taskid      任务ID
     * @param reason      任务失败的原因
     * @param trackerName 失败任务所在的TaskTracker
     */
    public void failedTask(TaskInProgress tip, TaskAttemptID taskid, String reason,
        ...
        TaskStatus status = TaskStatus.createTaskStatus(tip.isMapTask(),
                taskid,
                0.0f,
                tip.isMapTask() ? numSlotsPerMap : numSlotsPerReduce,
                state,
                reason,
                reason,
                trackerName, phase,
