 ...
 *
 * 定义了客户端和NameNode的通信协议，包含了34个方法，方法可以分为两组：
 * 1. Hadhoop文件系统相关
 *    * 包括文件/目录 元数据部分的操作
 *    * 写文件相关的操作：create append addBlock abandonBlock fsync complete
 *    * 读文件相关的操作：getBlockLocations reportBadBlocks
 * 2. 用于查询、设置HDFS的状态，多功能工具dfsadmin的实现依赖于这个接口
 *
 **********************************************************************/
@KerberosInfo(
...
public interface ClientProtocol extends VersionedProtocol {
     ...
     * @param src        path of the file being created.
     * @param clientName name of the current client.
     * @return 客户端获得追加的文件数据应该写往哪个数据块以及数据块的位置信息，接下来就可以联系数据节点，开始追加数据
