...
public class TaskTracker implements MRConstants, TaskUmbilicalProtocol, Runnable, TaskTrackerMXBean {
    ...
    static final String MR_CLIENTTRACE_FORMAT =
            "src: %s" +     // src IP
                    ", dest: %s" +  // dst IP
                    ", bytes: %s" + // byte count
                    ", op: %s" +    // operation
                    ", cliID: %s" +  // task id
                    ", duration: %s"; // duration
    static final Log ClientTraceLog = LogFactory.getLog(TaskTracker.class.getName() + ".clienttrace");
    ...
    Server taskReportServer = null; /*RPC  Server Task通过TPC向该Server汇报进度*/
    InterTrackerProtocol jobClient; /*RPC Client，用于与JobTracker通信*/

    private TrackerDistributedCacheManager distributedCacheManager;/*分布式缓存管理*/
    ...
    private final HttpServer server; /*将TaskTracker相关信息显示到Web前端*/

    volatile boolean shuttingDown = false;

    /*该节点上TaskAttemptID与TIP对应关系*/
    Map<TaskAttemptID, TaskInProgress> tasks = new HashMap<TaskAttemptID, TaskInProgress>();

    /*该节点上正在运行的TaskAttemptID与TaskInProgress的对应关系*/
    Map<TaskAttemptID, TaskInProgress> runningTasks = null;

    /*该节点上正在运行的作业列表，如果一个作业中的任务在节点上运行，则把该作业加入该数据结构*/
    Map<JobID, RunningJob> runningJobs = new TreeMap<>();
    ...
    /*标记着那些 正在 SHUFFLE阶段的 reduce任务 需要回滚到事件索引阶段，意思说 【reduce运行失败，需要重新下载任务？】*/
    private Set<TaskAttemptID> shouldReset = new HashSet<>();

    //dir -> DF
    Map<String, DF> localDirsDf = new HashMap<String, DF>();
    long minSpaceStart = 0;

    boolean acceptNewTasks = true;/*是否接收新任务，每次汇报时都要将该值告诉JobTracker*/
    long minSpaceKill = 0;
    ...
    private Localizer localizer;/*任务本地化，即准备任务运行环境*/
    private int maxMapSlots;/*TaskTracker上配置的Map slot数目*/
    private int maxReduceSlots;/*TaskTracker桑配置的Reduce slot数目*/
    private int failures;
    final long mapRetainSize;
    final long reduceRetainSize;

    private ACLsManager aclsManager; /*作业访问权限控制*/
    ...
    static private final int DEFAULT_OOB_HEARTBEAT_DAMPER = 1_000_000;
    private volatile int oobHeartbeatDamper;

    // Track number of completed tasks to send an out-of-band heartbeat
    private AtomicInteger finishedCount = new AtomicInteger(0);

    private MapEventsFetcherThread mapEventsFetcher; /*获取已运行的Map Task列表，为ReduceTask远程拷贝数据准备*/
    final int workerThreads;
    CleanupQueue directoryCleanupThread;
    private volatile JvmManager jvmManager; /*JVM管理*/

    private TaskMemoryManagerThread taskMemoryManager; /*任务内存监控*/
    ...
    private UserLogManager userLogManager; /*用户日志管理*/
     ...
     */
    private TaskController taskController; /*用于控制任务的初始化，终结和清理工作*/

    /**
     * Handle to the specific instance of the {@link NodeHealthCheckerService}
     */
    private NodeHealthCheckerService healthChecker;/*节点健康状况检查*/
   ...
   */
     ...
     */
    private Thread taskCleanupThread = new Thread(() -> {
        while (true) {
            try {
    ...
    static String getPrivateDirJobTokenFile(String user, String jobid) {
        return TT_PRIVATE_DIR + Path.SEPARATOR + getLocalJobTokenFile(user, jobid);
    }
    ...
    private FileSystem getFS(final Path filePath, JobID jobId, final Configuration conf) throws IOException, InterruptedException {
        RunningJob rJob = runningJobs.get(jobId);
        return rJob.ugi.doAs((PrivilegedExceptionAction<FileSystem>) () -> filePath.getFileSystem(conf));
    }
    ...
    synchronized void initialize() throws IOException, InterruptedException {
        ...
        this.jobClient = (InterTrackerProtocol) UserGroupInformation.getLoginUser()
                .doAs((PrivilegedExceptionAction<Object>) () -> RPC.waitForProxy(InterTrackerProtocol.class,
    ...
    private void createInstrumentation() {
        ...
        try {
            java.lang.reflect.Constructor<? extends TaskTrackerInstrumentation> c = metricsInst.getConstructor(TaskTracker.class);
    ...
    private class MapEventsFetcherThread extends Thread {
        /**
         * 为 在SHUFFLE 阶段的reduce任务创建 FetchStatus
         *
         * @return
         */
        private List<FetchStatus> reducesInShuffle() {
            List<FetchStatus> fList = new ArrayList<>();
            for (Entry<JobID, RunningJob> item : runningJobs.entrySet()) {
                ...
                synchronized (rjob) {
                    for (TaskInProgress tip : rjob.tasks) {
                        ...
                        if (!task.isMapTask() && task.getPhase() == Phase.SHUFFLE) {
                            if (rjob.getFetchStatus() == null) {
                                //this is a new job; we start fetching its map events
                                rjob.setFetchStatus(new FetchStatus(jobId, ((ReduceTask) task).getNumMaps()));
                            }
                            fList.add(rjob.getFetchStatus());
        ...
        public void run() {
            ...
            while (running) {
                try {
                    ...
                    synchronized (runningJobs) {
                        while ((fList = reducesInShuffle()).isEmpty()) {
                            try {
                                runningJobs.wait();
                    ...
                    /*为所有的reduce任务拉取所有的map任务完成事件*/
                    boolean fetchAgain = false; //flag signifying whether we want to fetch immediately again.
                    /*遍历所有作业 的readuce任务 的FetchStatus，拉取对应已完成的map任务*/
                    for (FetchStatus f : fList) {
                        ...
                        try {
                            ...
                            LOG.warn("Ignoring exception that fetch for map completion" +
                                    " events threw for " + f.jobId + " threw: " + StringUtils.stringifyException(e));
                        }
                        if (!running) {
                            break;
                        }
    ...
    RunningJob localizeJob(TaskInProgress tip) throws IOException, InterruptedException {
        ...
        try {
            /*作业本地化，如果作业文件很大，会导致rjob锁长时间不释放*/
            /*
            * <localizing,localized> : <false,false> -> <true,false> -> <true,true>
            * */
            synchronized (rjob) {
                /*如果作业没有被初始化*/
                if (!rjob.localized) {
                    /*如果前面有作业正在初始化，则当前线程等待*/
                    while (rjob.localizing) {
                        rjob.wait();
                    }
                    if (!rjob.localized) {/*如果没有初始化，则开始初始化*/
    ...
    Path initializeJob(final Task t, final RunningJob rjob, final InetSocketAddress ttAddr) throws IOException, InterruptedException {
        ...
        /*下载该作业的job.xml从HDFS */
        final Path localJobXMLFile = localizeJobConfFile(new Path(t.getJobFile()), userName, userFs, jobId);
         ...
         */
        /*接下来将作业剩下的初始化工作交 task-controller完成。
        * 我们需要令牌来支持 HDFS授权访问
        * */
        rjob.ugi.doAs((PrivilegedExceptionAction<Object>) () -> {
            try {
                final JobConf localJobConf = new JobConf(localJobXMLFile);
                ...
                JobLocalizer.writeLocalJobFile(localJobXMLFile, localJobConf);
                taskController.initializeJob(t.getUser(), jobId.toString(),
                        new Path(localJobTokenFile), localJobXMLFile, TaskTracker.this,
                        ttAddr);
            } catch (IOException e) {
    ...
    public TaskTracker(JobConf conf) throws IOException, InterruptedException {
        ...
        diskHealthCheckInterval = conf.getLong(DISK_HEALTH_CHECK_INTERVAL_PROPERTY, DEFAULT_DISK_HEALTH_CHECK_INTERVAL);

        UserGroupInformation.setConfiguration(originalConf);
        aclsManager = new ACLsManager(conf, new JobACLsManager(conf), null);
        this.jobTrackAddr = JobTracker.getAddress(conf);

        String infoAddr = NetUtils.getServerAddress(conf, "tasktracker.http.bindAddress",
                "tasktracker.http.port", "mapred.task.tracker.http.address");
        ...
        // let the jsp pages get to the task tracker, config, and other relevant objects
    ...
    private List<TaskCompletionEvent> queryJobTracker(IntWritable fromEventId,
        ...
        for (TaskCompletionEvent aT : t) {
            if (aT.isMap) {
                recentMapEvents.add(aT);
            }
        }
    ...
    /**
     * 为了提高任务的响应时间和资源利用率，TaskTracker一旦发现存在某个任务运行完成或者失败，
     * 就会立即缩短心跳间隔，以便将任务完成或失败的消息告诉JobTracker，进而快速重新分配任务，
     * 将这种特殊的心跳称为【带外心跳】。
     *
     * @param numFinishedTasks
     * @return
     */
    ...
    State offerService() throws Exception {
        ...
        while (running && !shuttingDown) {
            try {
                ...
                /*【步骤一】：判断是否到达心跳发送时间*/
                long remaining = (lastHeartbeat + getHeartbeatInterval(finishedCount.get())) - now;
                while (remaining > 0) {
                    ...
                    synchronized (finishedCount) {
                        ...
                        remaining = (lastHeartbeat + getHeartbeatInterval(finishedCount.get())) - now;
                ...
                if (now > (lastCheckDirsTime + diskHealthCheckInterval)) {
                    /*MapReduce计算过程中最重要的输出目录是参数mapred.local.dir指定的*/
                    /*中间结果存放目录，通常由多个目录组成，每个目录对应一个磁盘块*/
                    localStorage.checkDirs();
                ...
                if (!jobs.isEmpty()) {
                    synchronized (this) {
                        ...
                        /*通知所有 reducer的SHUFFLE阶段的reduce任务 重置fromEventID*/
                        synchronized (shouldReset) {
                            for (Entry<TaskAttemptID, TaskInProgress> entry : runningTasks.entrySet()) {
                ...
                killOverflowingTasks();/*剩余磁盘空间小于minSpaceKill时，寻找合适的任务将其杀掉以释放空间*/
    ...
    private void setIndexCache(IndexCache cache) {
        this.indexCache = cache;
    }

    /**
     * Build and transmit the heart beat to the JobTracker
     * 通过心跳向JobTracker汇报当前节点的运行时信息，包括TaskTracker的基本信息，节点资源使用情况和各个任务状态
     *
    ...
    private HeartbeatResponse transmitHeartBeat(long now) throws IOException {
        /*每隔 COUNTER_UPDATE_INTERVAL 发送一次 Counters*/
        ...
        synchronized (this) {
            /*存在空闲的map slot或者reduce slot，
            * 且磁盘剩余空间大于mapred.local.dir.minspacekill
            * */
            askForNewTask = ((status.countOccupiedMapSlots() < maxMapSlots
    ...
    private boolean enoughFreeSpace(long minSpace) throws IOException {
        return minSpace == 0 || minSpace < getFreeSpace();
    }
    ...
    private long getFreeSpace() throws IOException {
        ...
        for (String localDir : localDirs) {
            DF df;
            if (localDirsDf.containsKey(localDir)) {
                df = localDirsDf.get(localDir);
            } else {
                df = new DF(new File(localDir), fConf);
                localDirsDf.put(localDir, df);
            }
    ...
    private TaskLauncher mapLauncher; /*启动 Map任务*/
    private TaskLauncher reduceLauncher;/*启动 reduce任务*/
...
//TODO: are these calls really required
    ...
    private String localizeJobTokenFile(String user, JobID jobId) throws IOException {
        ...
        Path localJobTokenFile = lDirAlloc.getLocalPathForWrite(getPrivateDirJobTokenFile(user, jobId.toString()), jobTokenSize, fConf);
