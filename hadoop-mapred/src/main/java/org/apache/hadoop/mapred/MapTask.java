...
class MapTask extends Task {
    ...
    public void localizeConfiguration(JobConf conf)
        ...
        // jobConf-setup or a jobConf-cleanup task) and if the user wishes to run
    ...
    class TrackedRecordReader<K, V> implements RecordReader<K, V> {
        ...
        private Counters.Counter inputByteCounter; /*Map Task输入数据量（B）*/
        private Counters.Counter inputRecordCounter;/*读入记录数*/
        private Counters.Counter fileInputByteCounter;
        private InputSplit split; /*数据所在的分区*/
        ...
        TrackedRecordReader(InputSplit split, JobConf job, TaskReporter reporter) throws IOException {
            inputRecordCounter = reporter.getCounter(MAP_INPUT_RECORDS);
            inputByteCounter = reporter.getCounter(MAP_INPUT_BYTES);
            fileInputByteCounter = reporter.getCounter(FileInputFormat.Counter.BYTES_READ);
            ...
            bytesInPrev = getInputBytes(fsStats);/*默认是0*/
            rawIn = job.getInputFormat().getRecordReader(split, job, reporter);
            bytesInCurr = getInputBytes(fsStats);/*默认是0*/
            fileInputByteCounter.increment(bytesInCurr - bytesInPrev);/*初始是0*/
    ...
    private <T> T getSplitDetails(Path file, long offset)
        ...
        getCounters().findCounter(Counter.SPLIT_RAW_BYTES).increment(pos - offset);
        inFile.close();
        return split;
    }
    ...
    void runOldMapper(final JobConf job,
                      ...
                      TaskReporter reporter) throws IOException, InterruptedException, ClassNotFoundException {
        ...
        /*是否开启了跳跃坏记录模式，并初始化对应的记录阅读器*/
        RecordReader<INKEY, INVALUE> in = isSkipping() ?
                new SkippingRecordReader<>(inputSplit, umbilical, reporter) :
                new TrackedRecordReader<>(inputSplit, job, reporter);
    ...
    /**
     * Update the jobConf with details about the file split
     *
     * @param job        the jobConf configuration to update
    ...
    class DirectMapOutputCollector<K, V> implements MapOutputCollector<K, V> {
        ...
        public DirectMapOutputCollector(TaskUmbilicalProtocol umbilical,
            ...
            fileOutputByteCounter = reporter.getCounter(FileOutputFormat.Counter.BYTES_WRITTEN);
    ...
    class MapOutputBuffer<K extends Object, V extends Object> implements MapOutputCollector<K, V>, IndexedSortable {
        ...
        // k/v accounting
        private volatile int kvstart = 0;  // 表示存有数据的内存段初始位置
        private volatile int kvend = 0;    // marks beginning of collectable
        private int kvindex = 0;           // 表示未存储数据的内存段初始位置

        /*偏移量索引数组，用于保存key/value信息在位置索引【kvindices】中的偏移量*/
        private final int[] kvoffsets;     // indices into kvindices

        /*保存key/value所在的partition号，key开始位置和value开始位置*/
        ...
        private volatile int bufvoid = 0;  // 标记有效内存结束位置
        private int bufindex = 0;          // marks end of collected
        private int bufmark = 0;           // marks end of record

        /*kvbuffer，数据缓冲区，用于保存实际的key/value值，默认情况下最终可使用 io.sort.mb 的95%*/
        /*当缓冲区使用率超过 io.sort.spill.percent 后，便会触发线程SpillThread 将数据写入磁盘*/
        private byte[] kvbuffer;           // main output buffer
        private static final int PARTITION = 0; // partition offset in acct
        private static final int KEYSTART = 1;  // key offset in acct
        private static final int VALSTART = 2;  // val offset in acct

        private static final int ACCTSIZE = 3;    /*每对key/value 占用kvindices 中的三项*/
        private static final int RECSIZE = (ACCTSIZE + 1) * 4;  // 每对key/value 供占用kvoffsets和kvindices中的4个字节（16byte）
        ...
        public MapOutputBuffer(TaskUmbilicalProtocol umbilical, JobConf job, TaskReporter reporter)
            ...
            int maxMemUsage = sortmb << 20;/*将内存单位转换为字节，将 100MB 转为 100*1024*1024*/
            int recordCapacity = (int) (maxMemUsage * recper); /* 100*1024*1024 * 5%*/
            recordCapacity -= recordCapacity % RECSIZE;/*保证 recordCapacity 是4*4的整数倍*/
            kvbuffer = new byte[maxMemUsage - recordCapacity];/*kvbuffer 占了95%的内存空间，剩余5%空间分配给kvoffsets 和kvindices*/
            bufvoid = kvbuffer.length;
            recordCapacity /= RECSIZE; /*计算内存中最多保存【key/value数目】*/
            kvoffsets = new int[recordCapacity];/*kvoffset 占用1:3中的1*/
            kvindices = new int[recordCapacity * ACCTSIZE];/*kvindices 占用1:3中的3*/
                    ...
                    if (kvfull) {
                        try {
                            ...
                            throw (IOException) new IOException(
                                    "Collector interrupted while waiting for the writer", e).initCause(e);
                        }
                    }
                } while (kvfull);
            } finally {
        ...
        public int compare(int i, int j) {
            ...
            /*按照key值进行比较*/
            return comparator.compare(kvbuffer, kvindices[ii + KEYSTART], kvindices[ii + VALSTART] - kvindices[ii + KEYSTART],
                    kvbuffer, kvindices[ij + KEYSTART], kvindices[ij + VALSTART] - kvindices[ij + KEYSTART]);
        }
        ...
        protected class SpillThread extends Thread {
            ...
            public void run() {
                ...
                try {
                    while (true) {
                        spillDone.signal();/*解除spillDone.await()的等待，说明spill完成*/
                        while (kvstart == kvend) {
                            /*说明没有达到溢出阈值*/
                            spillReady.await();
                        }
                        try {
                            spillLock.unlock();
                            /*将环形缓冲区kvbuffer 中区间[bufstart,bufend]*/
                            sortAndSpill(); /*排序，然后将缓冲区 kvbuffer 中的数据写到磁盘上*/
                        } catch (Exception e) {
        ...
        private void sortAndSpill() throws IOException, ClassNotFoundException, InterruptedException {
            ...
            try {
                ...
                final Path spillFile = mapOutputFile.getSpillFileForWrite(numSpills, size);
                out = rfs.create(spillFile);/*在本地创建一个spill_0.out文件*/

                /*此次遍历的终点*/
                final int endPosition = (kvend > kvstart) ? kvend : kvoffsets.length + kvend;
                /*利用快速排序算法对缓冲区kvbuffer区间 [bufstart,bufend) 进行排序*/
                /*先按照分区编号partition进行排序，在按照key进行排序*/
                ...
                for (int i = 0; i < partitions; ++i) {
                    ...
                    try {
                        ...
                        writer = new Writer<K, V>(jobConf, out, keyClass, valClass, codec, spilledRecordsCounter);
                        if (combinerRunner == null) {
                            // spill directly
                            DataInputBuffer key = new DataInputBuffer();

                            /*找到对应的分区*/
                            /*经过排序后，数据以【分区为单位】聚集在一起，且同一分区内的所有数据按照key有序。*/
                            for (; spindex < endPosition
                                    && kvindices[kvoffsets[spindex % kvoffsets.length] + PARTITION] == i; ++spindex) {
                                final int kvoffset = kvoffsets[spindex % kvoffsets.length];
                                getVBytesForOffset(kvoffset, value);
                                key.reset(kvbuffer, kvindices[kvoffset + KEYSTART], (kvindices[kvoffset + VALSTART] - kvindices[kvoffset + KEYSTART]));
                                writer.append(key, value);
                            }
                        } else {/*如果用户设置了Combiner，则写入文件之前，对每个分区中的数据进行一次聚集操作*/
                            int spstart = spindex;
                            ...
                            /*eg：当前分区号是0（对应的索引范围是[spstart, spindex]）*/
                            if (spstart != spindex) {
                        ...
                        // record offsets
                        /*将分区数据的元信息写到内存索引数据结构总，
                        * 记录每个分区元信息包括在临时文件的偏移量、压缩前数据大小和压缩后数据大小
                        * */
            ...
            try {
                ...
                for (int i = 0; i < partitions; ++i) {
                    ...
                    try {
                        ...
                        writer = new Writer<K, V>(jobConf, out, keyClass, valClass, codec,
        ...
        private void mergeParts() throws IOException, InterruptedException,
            ...
            if (numSpills == 1) { //the spill is the final output
                ...
                if (indexCacheList.size() == 0) {
                    ...
                    indexCacheList.get(0).writeToFile(
                            new Path(filename[0].getParent(), "file.out.index"), jobConf);
                }
                return;
            }
            ...
            for (int i = indexCacheList.size(); i < numSpills; ++i) {
                ...
                indexCacheList.add(new SpillRecord(indexFileName, jobConf, null));
            }
            ...
            if (numSpills == 0) {
                ...
                try {
                    for (int i = 0; i < partitions; i++) {
                        ...
                        Writer<K, V> writer =
                                new Writer<K, V>(jobConf, finalOut, keyClass, valClass, codec, null);
                        writer.close();
                        rec.startOffset = segmentStart;
                        rec.rawLength = writer.getRawLength();
                        rec.partLength = writer.getCompressedLength();
                    ...
                    sr.writeToFile(finalIndexFile, jobConf);
                } finally {
                    finalOut.close();
                }
                return;
            }
            {
                ...
                for (int parts = 0; parts < partitions; parts++) {
                    ...
                    for (int i = 0; i < numSpills; i++) {
                        ...
                        Segment<K, V> s =
                                new Segment<K, V>(jobConf, rfs, filename[i], indexRecord.startOffset,
                    ...
                    //merge
                    @SuppressWarnings("unchecked")
                    RawKeyValueIterator kvIter = Merger.merge(jobConf, rfs,
                            keyClass, valClass, codec,
                            segmentList, jobConf.getInt("io.sort.factor", 100),
                            new Path(mapId.toString()),
                            jobConf.getOutputKeyComparator(), reporter,
                            null, spilledRecordsCounter);

                    //write merged output to disk
                    long segmentStart = finalOut.getPos();
                    Writer<K, V> writer =
                            new Writer<K, V>(jobConf, finalOut, keyClass, valClass, codec,
                                    spilledRecordsCounter);
                    if (combinerRunner == null || numSpills < minSpillsForCombine) {
                        Merger.writeFile(kvIter, writer, reporter, jobConf);
                    } else {
                        combineCollector.setWriter(writer);
                        combinerRunner.combine(kvIter, combineCollector);
                    }
                ...
                spillRec.writeToFile(finalIndexFile, jobConf);
                finalOut.close();
                for (int i = 0; i < numSpills; i++) {
