...
public abstract class TaskStatus implements Writable, Cloneable {
    ...
    private volatile State runState;/*任务的运行状态*/
    private String diagnosticInfo;/*诊断信息，一般为错误信息和运行异常信息*/
    private String stateString;/*字符串形式的运行状态*/
    private String taskTracker;/*该TaskTracker的名称，可唯一表示一个TaskTracker，形式如tracker_mymachine:50010*/
    private int numSlots;/*运行该TaskAttempt需要的slot数目，默认值为1*/

    private long startTime;/*TaskAttempt开始时间*/
    private long finishTime;/*TaskAttempt完成时间*/
    private long outputSize = -1L;/*TaskAttempt输出数据量*/

    private volatile Phase phase = Phase.STARTING;
    private Counters counters;/*该任务中定义的所有计数器（系统自带的和用户自定义的）*/
    private boolean includeCounters;/*是否包含计数器，计数器信息每隔60s 发送一次*/
    ...
    boolean inTaskCleanupPhase() {
        return this.phase == Phase.CLEANUP
                && (this.runState == State.FAILED_UNCLEAN || this.runState == State.KILLED_UNCLEAN);
    }
