...
public class AccessControlList implements Writable {
    ...
    static {                                      // register a ctor
        WritableFactories.setFactory(AccessControlList.class, AccessControlList::new);
    }
    ...
    private void buildACL(String aclString) {
        ...
        if (aclString.contains(WILDCARD_ACL_VALUE) &&
            ...
            if (userGroupStrings.length >= 1) {
                List<String> usersList = new LinkedList<String>(Arrays.asList(userGroupStrings[0].split(",")));
    ...
    private static void cleanupList(List<String> list) {
    ...
    private static void addToSet(Set<String> set, List<String> list) {
        for (String s : list) {
            set.add(s);
        }
    }
