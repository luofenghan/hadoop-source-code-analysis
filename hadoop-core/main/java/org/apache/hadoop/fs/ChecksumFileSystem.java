...
public abstract class ChecksumFileSystem extends FilterFileSystem {
    ...
    private static class ChecksumFSInputChecker extends FSInputChecker {
        ...
        protected int readChunk(long pos, byte[] buf, int offset, int len,
            ...
            boolean eof = false;
            if (needChecksum()) {//先进行校验和文件的读取
                try {
                    long checksumPos = getChecksumFilePos(pos);
                    if (checksumPos != sums.getPos()) { //校验和文件的位置
                        sums.seek(checksumPos); //调到校验和文件的读取位置
                    }
                    sums.readFully(checksum); //读满整个checksum缓冲区，
         ...
         * @throws IOException if an I/O error occurs or seeks after EOF
    ...
    private static class ChecksumFSOutputSummer extends FSOutputSummer {
        ...
        public ChecksumFSOutputSummer(ChecksumFileSystem fs,
            ...
            this.sums = fs.getRawFileSystem().create(fs.getChecksumFile(file), true,
                    sumBufferSize, replication, blockSize);
            //写入校验文件的文件头，包括校验方法和进行校验的数据块大小
