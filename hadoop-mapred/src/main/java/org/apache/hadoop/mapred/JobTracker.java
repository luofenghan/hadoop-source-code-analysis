...
public class JobTracker implements MRConstants, InterTrackerProtocol,
    ...
    // Approximate number of heartbeats that could arrive JobTracker in a second
    ...
    /*用于构造集群的网络拓扑结构，将节点地址（IP或Host）映射成网络位置*/
    ...
    private final TaskScheduler taskScheduler;/*任务调度器对象*/
                        ...
                        synchronized (taskTrackers) {
                            synchronized (trackerExpiryQueue) {
                                ...
                                while ((trackerExpiryQueue.size() > 0) &&
                                    ...
                                    /*TaskTracker或许会通过其他方式离开 taskTrackers*/
                                    /*因此，taskTrackers中存储的状态或许为空*/
                                    /*意味着TaskTracker已经被销毁*/
                                    if (newProfile != null) {
    ...
    private static class FaultInfo {
        ...
        FaultInfo(long time, int numFaultBuckets, long bucketWidth) {
            ...
            /*经过初始化后，lastRotated 是 bucketWidth 的整数倍*/
    ...
    private class FaultyTrackersInfo {
        ...
        void incrementFaults(String hostName) {
            synchronized (potentiallyFaultyTrackers) {
                ...
                if (exceedsFaults(fi, now)) {
                    ...
                    blacklistTracker(hostName, reason, ReasonForBlackListing.EXCEEDING_FAILURES, true);
                }
            }
        }
        ...
        private FaultInfo getFaultInfo(String hostName, boolean createIfNecessary) {
            ...
            synchronized (potentiallyFaultyTrackers) {
                ...
                if (fi == null && createIfNecessary) {
                    fi = new FaultInfo(clock.getTime(), NUM_FAULT_BUCKETS, TRACKER_FAULT_BUCKET_WIDTH_MSECS);
        ...
        void markTrackerHealthy(String hostName) {
            synchronized (potentiallyFaultyTrackers) {
                ...
                if (fi != null) {
                    /*一个TaskTracker可能同时在 blacklisted和 graylisted中，所以都需要检查一下*/
                    if (fi.isGraylisted()) {
        ...
        void setNodeHealthStatus(String hostName, boolean isHealthy, String reason, long timeStamp) {
            ...
            if (!isHealthy) {
                ...
                synchronized (potentiallyFaultyTrackers) {
                    blacklistTracker(hostName, reason, ReasonForBlackListing.NODE_UNHEALTHY, false);
                }
            } else {
                if (getFaultInfo(hostName, false) != null) {
                    unBlacklistTracker(hostName, ReasonForBlackListing.NODE_UNHEALTHY, false, timeStamp);
                }
            }
        }
    }
    ...
    class RecoveryManager {
        ...
        private JobStatusChangeEvent updateJob(JobInProgress jip,
            ...
            jip.updateJobInfo(job.getLong(Keys.SUBMIT_TIME),
                ...
                try {
                   ...
                   */
                    job = new JobInProgress(JobTracker.this, conf,
                            new JobInfo(id, new Text(user), new Path(getStagingAreaDirInternal(user))),
    ...
    /*作业恢复管理，即JobTracker启动时，恢复上次停止时正在运行的作业，并恢复各个任务的运行状态*/
     ...
     */
    TreeSet<TaskTrackerStatus> trackerExpiryQueue = new TreeSet<>((p1, p2) -> {
    ...
    final HttpServer infoServer;/*将Job、Task和TaskTracker相关信息显示到Web前端*/
    int infoPort;

    Server interTrackerServer;/*RPC Server*/
    ...
    JobTracker(final JobConf conf, String identifier, Clock clock, QueueManager qm) throws IOException, InterruptedException {
        ...
        /*Job Tracker RPC Server  Address */
        ...
        getMROwner().doAs((PrivilegedExceptionAction<Boolean>) () -> {
        ...
        while (!Thread.currentThread().isInterrupted()) {
            try {
                ...
                if (fs == null) {
                    fs = getMROwner().doAs((PrivilegedExceptionAction<FileSystem>) () -> FileSystem.get(conf));
                }
        ...
        FileSystem historyFS = getMROwner().doAs((PrivilegedExceptionAction<FileSystem>) () -> {
            JobHistory.initDone(conf, fs);
            final String historyLogDir =
                    JobHistory.getCompletedJobHistoryLocation().toString();
            infoServer.setAttribute("historyLogDir", historyLogDir);

            infoServer.setAttribute("serialNumberDirectoryDigits", JobHistory.serialNumberDirectoryDigits());

            infoServer.setAttribute("serialNumberTotalDigits", JobHistory.serialNumberTotalDigits());
    ...
    void markCompletedTaskAttempt(String taskTracker, TaskAttemptID taskid) {
        // tracker --> taskid
        Set<TaskAttemptID> taskset = trackerToMarkedTasksMap.computeIfAbsent(taskTracker, k -> new TreeSet<TaskAttemptID>());
        taskset.add(taskid);
        if (LOG.isDebugEnabled()) {
    ...
    public synchronized HeartbeatResponse heartbeat(TaskTrackerStatus status,
        ...
        /*检查是否允许该TaskTracker连接JobTracker*/
        ...
        if (restarted) {
            ...
            /*启动TaskTracker容错机制 以检查它是否处于健康状态*/
        ...
        if (!initialContact) {
            /*如果不是第一次连接，并且之前心跳响应为null，则说明状态不一致，需要重启TaskTracker*/
            if (prevHeartbeatResponse == null) {
                /*说明是一个旧的Tracker 向新的TaskTracker发送的心跳响应*/
                if (hasRestarted()) {
                    addRestartInfo = true;
        ...
        if (recoveryManager.shouldSchedule() && acceptNewTasks && !isBlacklisted) {
            /*
             * 如果一个正常的TaskTracker（不在黑名单中），并且有空闲的slot,
             * 则JobTracker会为该TaskTracker分配新任务，任务顺序是
             * <pre>
             *     1. 先辅助性任务
             *          - job-cleanup-task
             *          - task-cleanup
    ...
    public int getNextHeartbeatInterval() {
        ...
        /*获取集群总数，即当前集群规模*/
        int clusterSize = getClusterStatus(false).getTaskTrackers();
        return Math.max((int) (1000 * HEARTBEATS_SCALING_FACTOR * Math.ceil((double) clusterSize / NUM_HEARTBEATS_IN_SECOND)),
     ...
     * 检查一个TaskTracker在include列表里，但不在exlclude列表里
     */
    ...
    /**
     * 用给定的TaskTracker更新上次的记录状态，在入口方法中必须加锁
     *
    ...
    private boolean updateTaskTrackerStatus(String trackerName, TaskTrackerStatus status) {
        ...
        if (status != null) {
            ...
            if (!alreadyPresent) {/*新添加的TaskTracker*/
    ...
    private void updateNodeHealthStatus(TaskTrackerStatus trackerStatus, long timeStamp) {
        ...
        synchronized (faultyTrackers) {
            faultyTrackers.setNodeHealthStatus(trackerStatus.getHost(), status.isNodeHealthy(), status.getHealthReport(), timeStamp);
        }
    }
    ...
    private synchronized boolean processHeartbeat(TaskTrackerStatus trackerStatus,
        ...
        synchronized (taskTrackers) {
            synchronized (trackerExpiryQueue) {
                ...
                /*如果 seenBefore ==false，说明第一次连接，*/
                TaskTracker taskTracker = getTaskTracker(trackerName);
                if (initialContact) {
                    // If it's first contact, then clear outany state hanging around
                    /*如果是第一次连接，清除相关的记录，*/
                    if (seenBefore) {
    ...
    private void addJobForCleanup(JobID id) {
        for (String taskTracker : taskTrackers.keySet()) {
            ...
            synchronized (trackerToJobsToCleanup) {
                Set<JobID> jobsToKill = trackerToJobsToCleanup.computeIfAbsent(taskTracker, k -> new HashSet<JobID>());
                jobsToKill.add(id);
            }
        }
    }
    ...
    private synchronized List<TaskTrackerAction> getTasksToSave(TaskTrackerStatus tts) {
        /*得到TaskTracker上各个任务的运行状态*/
        List<TaskStatus> taskStatuses = tts.getTaskReports();
        if (taskStatuses != null) {
            List<TaskTrackerAction> saveList = new ArrayList<>();
     ...
     */
    /**
     * @param jobId        作业的id，客户端通过getNewJobId函数为作业获取唯一的ID
    ...
    public String getStagingAreaDir() throws IOException {
        try {
            ...
            return getMROwner().doAs((PrivilegedExceptionAction<String>) () -> getStagingAreaDirInternal(user));
        } catch (InterruptedException ie) {
    ...
    public synchronized void failJob(JobInProgress job) {
        ...
        JobStatus newStatus = (JobStatus) job.getStatus().clone();
        if (prevStatus.getRunState() != newStatus.getRunState()) {
            updateJobInProgressListeners(new JobStatusChangeEvent(job, EventType.RUN_STATE_CHANGED, prevStatus, newStatus));
        }
    }
   ...
   */
    ...
    void updateTaskStatuses(TaskTrackerStatus status) {
        ...
        for (TaskStatus report : status.getTaskReports()) {
            ...
            if (job == null) {
                ...
                synchronized (trackerToJobsToCleanup) {
                    Set<JobID> jobs = trackerToJobsToCleanup.computeIfAbsent(trackerName, k -> new HashSet<>());
            ...
            if (!job.inited()) {
                ...
                synchronized (trackerToTasksToCleanup) {
                    Set<TaskAttemptID> tasks = trackerToTasksToCleanup.computeIfAbsent(trackerName, k -> new HashSet<TaskAttemptID>());
                    tasks.add(taskId);
                }
                continue;
            }
    ...
    void lostTaskTracker(TaskTracker taskTracker) {
        ...
        if (lostTasks != null) {
            ...
            Set<JobInProgress> jobsWithFailures = new HashSet<>();
            for (TaskAttemptID taskId : lostTasks) {
                TaskInProgress tip = taskidToTIPMap.get(taskId);
                JobInProgress job = tip.getJob();

                if (!tip.isComplete() || (tip.isMapTask() && !tip.isJobSetupTask() && job.desiredReduces() != 0)) {
                    /*如果某个任务没有完成 或者 （该任务是Map任务 并且 不是作业设置任务 并且 该作业有reduce任务）*/
                    if (job.getStatus().getRunState() == JobStatus.RUNNING ||
                            job.getStatus().getRunState() == JobStatus.PREP) {
                        /*如果当前任务处于运行状态 并且 不是setup任务和清理任务，则标记为已杀死但未清理状态*/
                        /*否则标记为已杀死*/
                        ...
                        /*JobInProgress将该任务标记为失败*/
                        job.failedTask(tip, taskId, ("Lost task tracker: " + trackerName),
                                (tip.isMapTask() ? TaskStatus.Phase.MAP : TaskStatus.Phase.REDUCE), killState, trackerName);
                        jobsWithFailures.add(job);
                    }
                } else {
                    /*如果对应的reduce任务已经完成，则不需要标记为失败，因为其结果已经写入dfs*/
                    /*如果某个作业的map任务已经完成，并且不需要启动相关的reducer任务，则不需要标记为失败*/
    ...
    private void checkMemoryRequirements(JobInProgress job)
        ...
        if (invalidJob) {
            StringBuilder jobStr = new StringBuilder()
                    ...
                    .append("(")
                    .append(maxMemForMapTask)
                    .append(" memForMapTasks ")
    ...
    synchronized private String getReasonsForBlackOrGraylisting(String host, boolean gray) {
