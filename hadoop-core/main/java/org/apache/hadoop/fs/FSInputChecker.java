...
abstract public class FSInputChecker extends FSInputStream {
     ...
     */
    protected Path file;
    private Checksum sum; //校验和
    private boolean verifyChecksum = true;
    private byte[] buf;//暂时保存输入数据的缓冲区buf
    private int count; //buf已使用空间计数
    private byte[] checksum; //检验和缓冲区
    private int pos; //已读取数据位置
    private int numOfRetries; //出错重试次数
    private long chunkPos = 0; //当前数据块位置
    ...
    public synchronized int read(byte[] b, int off, int len) throws IOException {
        ...
        int n = 0;
        for (; ; ) {
            int nread = read1(b, off + n, len - n);//循环读取数据
            if (nread <= 0)
                return (n == 0) ? nread : n;
            n += nread;
            if (n >= len)
    ...
    private int read1(byte b[], int off, int len)
            throws IOException {
        int avail = count - pos; //计算buf内剩余未读取字节数
        if (avail <= 0) {//如果缓冲区数据已读完
            if (len >= buf.length) {//如果用户缓冲区大于内置缓冲区
                //读取一块数据到用户缓冲区，避免数据拷贝
                return readChecksumChunk(b, off, len);
            } else {
                // 读取一整块数据到FSInputChecker的缓冲区
                fill();
                if (count <= 0) {
                    return -1;
                } else {
    ...
    public synchronized void seek(long pos) throws IOException {
        ...
        // 如果要定位的数据正好处于当前缓冲区内，那么只需要简单的改变下次读取数据的指针pos即可。
